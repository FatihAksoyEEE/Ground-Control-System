...
contextBridge.exposeInMainWorld('electronAPI', {
    ...
    onSerialStatus: (callback) =>
        ipcRenderer.on('serial-status', (event, value) => {
            // Status updates (connected, disconnected, error)
            callback(value);
        }),
    ...
    onSerialData: (callback) =>
        ipcRenderer.on('serial-data', (event, data) => {
            // Data received from Serial (ArrayBuffer)
            callback(data);
        }),

    // --- UI CONTROL ---
    onTogglePanel: (callback) =>
        ipcRenderer.on('toggle-panel', (event, data) => {
            callback(data);
        }),
});
