  ...
  openPort(portPath, baudRate, resolve, reject) {
    ...
    this.port.open((err) => {
      ...
      this.port.on("error", (err2) => {
        ...
        if (this.mainWindow && !this.mainWindow.isDestroyed()) {
          this.mainWindow.webContents.send("serial-status", { connected: false, error: err2.message });
        }
      });
      resolve(true);
    });
...
const createWindow = () => {
  mainWindow = new BrowserWindow({
    ...
    // Uygulama arka plan rengiyle uyumlu
    webPreferences: {
      preload: path.join(__dirname, "preload.js"),
      nodeIntegration: false,
      contextIsolation: true,
      sandbox: false
      // Node modüllerine preload üzerinden erişim için bazen gerekebilir
    },
...
app.whenReady().then(() => {
  ...
  ipcMain.handle("serial:connect", async (event, { path: path2, baudRate }) => {
    if (serialManager) {
      try {
        await serialManager.connect(path2, parseInt(baudRate));
        return { success: true };
      } catch (err) {
